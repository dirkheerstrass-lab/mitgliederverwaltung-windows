...
- [x] **Excel-Mitgliederimport mit freier Spaltenzuordnung** (v1.03): `ExcelImportDialog`, aus "Neues Mitglied" heraus aufrufbar ("Mitglieder aus Excel importieren..."). Beliebige Excel-Datei auswählen, jede Spalte per Dropdown einem internen Feld zuordnen (automatischer Vorschlag bei exakter Namensübereinstimmung, frei überschreibbar), Live-Vorschau der ersten 5 Zeilen, Datumsnormalisierung wiederverwendet `mitglieder._normalisiere_importiertes_datum()`, Import zeilenweise über `validate_member()` geprüft (kein Alles-oder-Nichts, Ergebnis "N erfolgreich / N Zeilen übersprungen mit Grund"). Der einfache, feste Web-App-Import (`import_members_from_excel`) wurde dadurch nicht zusätzlich portiert — die flexible Variante deckt seinen Anwendungsfall (Datei mit exakt passenden Spaltennamen) mit ab.
