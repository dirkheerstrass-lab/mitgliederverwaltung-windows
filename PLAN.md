...
- [ ] **Fortschrittsbalken beim Anhang-Upload**: `MemberEditDialog._anhang_hinzufuegen()` (`src/gui.py`, ca. Zeile 933) ruft `mitglieder.save_attachment()` synchron pro Datei auf — bei mehreren/großen Dateien (Fotos, PDFs) friert die UI dabei kommentarlos ein, ohne Rückmeldung, ob/wann der Vorgang fertig ist. Gewünscht: sichtbarer Fortschrittsbalken während des Hochladens. Umsetzungsidee: `QProgressDialog` (unbestimmter Modus/`setRange(0,0)` reicht für einzelne größere Dateien; bei mehreren ausgewählten Dateien `setRange(0, len(pfade))` und nach jeder Datei `setValue()` erhöhen, da `save_attachment()` selbst kein Fortschritts-Callback hat und Kopieren pro Datei nur als ganzer Schritt zählt) mit "Abbrechen"-Möglichkeit zwischen Dateien. Bei sehr großen Einzeldateien (echter Byte-Fortschritt) müsste `mitglieder.save_attachment()` (gemeinsame Kopie mit der Web-App, siehe Hinweis zu `mitglieder.py`/`mailer.py` oben) angepasst werden, um einen Fortschritts-Callback zu unterstützen — dafür beim Umsetzen abwägen, ob der einfache dateiweise Fortschritt genügt oder echter Byte-Fortschritt gewünscht ist. Gleiche Überlegung gilt für den Foto-Upload (`_foto_hochladen()`) und den Anhang-Upload beim "Neues Mitglied"-Formular (Zeile ~1767).
